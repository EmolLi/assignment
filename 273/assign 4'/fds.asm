...
# temporary register $t7 is changed to store i
# temporary register $f10 is changed to store (a+i*epsilon)
# temporary register $f9 is changed to store a
	
	#load a to f9
	mov.s	$f9, $f12
	#t7 is i, initialize it to zero
	li	$t7, 0
	...
	fLoop:
		...
		#convert i to float
		mtc1	$t7, $f4
		cvt.s.w $f4, $f4		#f4 <- i in float
		mul.s	$f4, $f4, $f14	#f4 <- i*epsilon
		add.s	$f10, $f9, $f4	#f10 <- a+i*epsilon
		
		
		#if p(a+i*epsilon)>=0 return a+i*epsilon
		#save $ra
		addi	$sp, $sp, -4
		sw	$ra, 0($sp)
	
		#calulate p(a+i*epsilon), the value is in f0
		mov.s	$f12, $f10
		jal	evaluate
		...
		#compare p(a+i*epsilon) with 0
		mtc1	$zero, $f4	#f4 <- 0
		c.lt.s	$f4, $f0	#if 0 < p(a+i*epsilon)
		#if true, jump out of the loop to return a+i*epsilon
		bc1t	return
		#else i++, continue the loop
		addi	$t7, $t7, 1
		j	fLoop
